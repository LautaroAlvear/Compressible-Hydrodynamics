...
rmsfile = f"RMS/rms_CMHD_equilibrium_{nu}_{kappa}_{eta}_{sys.argv[4]}.dat"
EnergyFile = f"Energies/Energies_CMHD_equilibrium_{nu}_{kappa}_{eta}_{sys.argv[4]}.dat"
filesnapshots = f"CMHD_equilibrium_{safe_float(nu)}_{safe_float(kappa)}_{safe_float(eta)}_{safe_float(float(sys.argv[4]))}_slices"
...
B["g"][1] = -constant*np.sin(theta)*(-1.60864+24.4376*r**2-81.6573*r**4+114.293*r**6-73.9407*r**8+18.3572*r**10
                               +r*(7.2286-17.211*r**2+9.37043*r**4)*np.cos(theta)*np.cos(theta)*np.sin(theta)*np.sin(theta))
    
B['g'][2] = constant*np.cos(theta)*(-1.6086+12.2188*r**2-27.2191*r**4+28.5733*r**6-14.7881*r**8+3.059*r**10
                               +1.3386*r*(1.8-2.5714*r**2+r**4)*(np.cos(theta)*np.cos(theta)-np.sin(theta)*np.sin(theta))*np.sin(theta)*np.sin(theta))
    
B['g'][0] = constant*71.1803*(r-1)**2 *r**4*np.sin(theta-np.pi/5)*np.sin(theta)
...
problem = d3.IVP([lnrho, u, T, tau_u,B], namespace=locals())
#--------------Equations-----------------------------------#
...
problem.add_equation("dt(T)  = - u@grad(T)-(Gamma-1)*T*div(u)+2*nu*S2+eta*j2/rho")

problem.add_equation("dt(B) = curl(cross(u,B))")
...
slices.add_task(np.sqrt(d3.DotProduct(u,u)),name="magu",layout="g")
...
slices.add_task(B,layout="g",name="B")
